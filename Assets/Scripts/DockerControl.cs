...
public class DockerControl : MonoBehaviour
{
    ...
    private void CheckBoxInit(){
        ...
        foreach (var launchFile in scpt_cfg.LaunchItems)
        {
            ...
            checkbox.GetComponent<ToggleControl>().noInit();
            ...
            newHeight += checkbox.GetComponent<ToggleControl>().getHeight();
            // UnityEngine.Debug.Log($"{newHeight}");
        }
        foreach (var launchFile in scpt_cfg.LaunchOptions)
        {
            // 创建复选框实例
            GameObject checkbox = Instantiate(checkboxPrefab,contentPanel);
            checkbox.transform.SetParent(contentPanel);
            ToggleControl toggle_scpt = checkbox.GetComponent<ToggleControl>();
            var launchOpts = launchFile.Value.Opt;
            toggle_scpt.initOpt(ref launchOpts, scpt_cfg);
            ...
            checkbox.GetComponent<Toggle>().onValueChanged.AddListener((bool isOn) =>
            {
                    ...
                    string keyToFind = checkbox.GetComponentInChildren<Text>().text;
                    ConfigLoader.LaunchOptionCheck item;
                    if (scpt_cfg.LaunchOptions.TryGetValue(keyToFind, out item)){
                        if (isOn){item.Enabled = true;}
                        else{item.Enabled = false;} 
                        scpt_cfg.LaunchUpdate();
                    }
            ...
            newHeight += toggle_scpt.getHeight();
        }
