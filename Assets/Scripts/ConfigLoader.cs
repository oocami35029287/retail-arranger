...
public class ConfigLoader : MonoBehaviour
{
    ...
    [System.NonSerialized]
    public Dictionary<string, LaunchOptionCheck> LaunchOptions;
    [System.NonSerialized]
    ...
    public class LaunchOption
    {
        ...
        public bool Enabled { get; set; }
        public int Delay { get; set; }
        public List<LaunchOpt> Opt { get; set; }
    }

    public class LaunchOptionCheck
    {
        ...
        public bool Enabled { get; set; }
        public int Delay { get; set; }
        public List<OptCheck> Opt { get; set; }
    }
    [System.Serializable]
    ...
    public class ConfigData
    {
        ...
        public Dictionary<string, LaunchOption> LaunchOptions { get; set; }
    }
    ...
    public void LoadConfig()
    {
        ...
        if (File.Exists(fullPath))
        {
            ...
            //2
            LaunchOptions = new Dictionary<string,LaunchOptionCheck>();
            foreach(var launchfile in configData.LaunchOptions){
                LaunchOptionCheck launchOption = new LaunchOptionCheck();
                launchOption.Opt = new List<LaunchOptionCheck.OptCheck>();
                launchOption.Enabled = launchfile.Value.Enabled;
                launchOption.Delay = launchfile.Value.Delay;
                foreach(var opt in launchfile.Value.Opt){
                    LaunchOptionCheck.OptCheck optcheck = new LaunchOptionCheck.OptCheck();
                    optcheck.Selected = false;
                    optcheck.Name = opt.Name;
                    optcheck.Workspace = opt.Workspace;
                    optcheck.Command = opt.Command;
                    launchOption.Opt.Add(optcheck);
                }
                LaunchOptions.Add(launchfile.Key,launchOption);
            }
    ...
    public void LaunchUpdate(){
        ...
        foreach(var item in LaunchOptions){
            LaunchOptionCheck itm = item.Value;
            if(itm.Enabled){
                foreach(var opt in item.Value.Opt){
                    if(opt.Selected){
                        LaunchItemsForRun.Add(new LaunchItem
                            {
                                Enabled = itm.Enabled,
        ...
        // foreach(var item in LaunchItemsForRun){
