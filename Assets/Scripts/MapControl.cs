...
public class MapControl : MonoBehaviour
{
    ...
    public Button mapFolderBtn;
    public Button sceneFolderBtn;
    void Start(){
        ...
        mapFolderBtn.onClick.AddListener(mapFolder);
        sceneFolderBtn.onClick.AddListener(sceneFolder);
    ...
    private void mapFolder()
    {
        ...
        process.StartInfo.Arguments = Application.dataPath + "/config/maps/";
        process.Start();
        process.WaitForExit();
        RefreshMapOption();
    }
    private void sceneFolder()
    {        
        Process process = new Process();
        process.StartInfo.FileName = "xdg-open";
        process.StartInfo.Arguments = scpt_cfg.FileDir+scpt_cfg.sceneDir;
        process.Start();
        process.WaitForExit();
        RefreshSceneOption();
    }
    ...
    private void LoadMapYaml(string mapYaml){
        ...
        if (File.Exists(yamlPath)){
            ...
            //UnityEngine.Debug.Log("File Content: " + yamlText);
            ...
            UnityEngine.Debug.LogError("File not found: " + yamlPath);
        }
    ...
    private IEnumerator LoadMap(string map){
        ...
        RawImage imageShow = mapImage.transform.Find("mapImageShow").GetComponent<RawImage>();
        imageShow.texture = www.texture;
        imageShow.rectTransform.sizeDelta = new Vector2(www.texture.width,www.texture.height);
        if(map.StartsWith("gen_world")){
            imageShow.rectTransform.anchoredPosition = new Vector2(www.texture.width/2,www.texture.height/2);
        }
        else{
            imageShow.rectTransform.anchoredPosition = new Vector2(0,0);
        }
    }
    private void LoadScene(string scene){
        ...
        if (File.Exists(scenePath)){
            ...
            //UnityEngine.Debug.Log(sceneText);
            ...
            foreach (XmlNode obstacleNode in obstacleNodes)
            {
                ...
                //UnityEngine.Debug.Log($"Obstacle: x1 = {x1}, x2 = {x2}, y1 = {y1}, y2 = {y2}");
            ...
            UnityEngine.Debug.LogError("File not found: " + scenePath);
        }
    ...
    private void RotateRobot(string inputText){
        ...
        if (float.TryParse(inputText, out floatValue)){
            ...
            UnityEngine.Debug.Log("输入文本：" + floatValue);
        }
        else{
            // Conversion failed, handle the error
            UnityEngine.Debug.LogError("Failed to convert input text to float.");
        }
    }
